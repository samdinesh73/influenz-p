...
export const completeProfile = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const token = jwt.sign(
      ...
      process.env.JWT_SECRET || "super-secret-influenz-key-2026",
      { expiresIn: "7d" }
    );
...
export const authenticateToken = (req: any, res: Response, next: any) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET || "super-secret-influenz-key-2026", (err: any, decoded: any) => {
    if (err) {
...
export const getAdminData = async (req: any, res: Response): Promise<void> => {
  try {
...
export const toggleVerifyUser = async (req: any, res: Response): Promise<void> => {
  try {
    if (req.user.role !== "master_admin") {
...
export const moderateCampaign = async (req: any, res: Response): Promise<void> => {
  try {
    if (req.user.role !== "master_admin") {
      res.status(403).json({ error: "Access denied." });
      return;
...
export const createInvitation = async (req: any, res: Response): Promise<void> => {
  try {
    const userId = req.user.userId;
...
export const getInvitations = async (req: any, res: Response): Promise<void> => {
  try {
    const userId = req.user.userId;
...
export const updateInvitationStatus = async (req: any, res: Response): Promise<void> => {
  try {
    const userId = req.user.userId;
...
export const getApplications = async (req: any, res: Response): Promise<void> => {
  try {
    ...
    const campaigns = await prisma.campaign.findMany();
    const influencers = await prisma.influencer.findMany({ include: { user: true } });
